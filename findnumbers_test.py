...
def test_find_numbers_sm_bool():
    nums = find_numbers_sm_bool("")
    assert list(nums) == []

    nums = find_numbers_sm_bool("no numbers here")
    assert list(nums) == []

    nums = find_numbers_sm_bool("0")
    assert list(nums) == [0]

    nums = find_numbers_sm_bool("012")
    assert list(nums) == [12]

    nums = find_numbers_sm_bool("0 1 2")
    assert list(nums) == [0, 1, 2]

    nums = find_numbers_sm_bool("0, 1, 2,")
    assert list(nums) == [0, 1, 2]

    nums = find_numbers_sm_bool("(123)")
    assert list(nums) == [123]

    nums = find_numbers_sm_bool('2some34numbers56here78"')
    assert list(nums) == [2, 34, 56, 78]

    nums = find_numbers_sm_bool("-1+2")
    assert list(nums) == [1, 2]
